...
import com.cortalabs.osrs.analytics.beans.RoleIndex;
import com.cortalabs.osrs.analytics.beans.AnalyticsStatus;
import com.cortalabs.osrs.analytics.beans.ParticipantWithCompetition;
import com.cortalabs.osrs.analytics.beans.GroupMemberAddition;
import com.cortalabs.osrs.analytics.beans.Member;
import com.cortalabs.osrs.analytics.beans.PlayerInfo;
import com.cortalabs.osrs.analytics.beans.AnalyticsPlayerUpdate;
...
public class AnalyticsClient
{
	...
	@Inject
	public AnalyticsClient(Gson gson, AnalyticsPlugin plugin, Client client, OkHttpClient okHttpClient)
	{
		...
		String pluginVersion = AnalyticsPlugin.getPluginVersion();
		String runeliteVersion = RuneLiteProperties.getVersion();

		userAgent = "OSRSAnalyticsRuneLitePlugin/" + pluginVersion + " " +
	...
	public void submitNameChanges(NameChangeEntry[] changes)
	{
		...
		sendRequest(request);
		log.info("Submitted {} name changes to OSRS Analytics", changes.length);
	}
	...
	{
		sendRequest(request, new AnalyticsCallback(consumer));
	}

	void sendRequest(Request request, Consumer<Response> consumer, Consumer<Exception> exceptionConsumer)
	{
		sendRequest(request, new AnalyticsCallback(consumer, exceptionConsumer));
	}
	...
	private void importMembersCallback(Response response)
	{
		...
		postEvent(new AnalyticsGroupSynced(groupInfo, true));
	}
	...
	private void syncClanMembersCallBack(Response response)
	{
		...
		if (response.isSuccessful())
		{
			...
			postEvent(new AnalyticsGroupSynced(data));
			this.isSyncing = false;
		}
		else if (response.code() == 429)
		{
			log.error("osrs-analytics: reached api limits while syncing clan members");
			this.isSyncing = false;
		}
		else if (response.code() == 403)
		{
			AnalyticsStatus data = parseResponse(response, AnalyticsStatus.class);
	...
	private void playerOngoingCompetitionsCallback(String username, Response response)
	{
		...
		if (response.isSuccessful())
		{
			...
			postEvent(new AnalyticsOngoingPlayerCompetitionsFetched(username, comps));
			showRetry = false;
		}
		else if (response.code() == 429)
		{
			log.error("osrs-analytics: reached api limits while fetching ongoing competitions");
		}
		else
		{
			AnalyticsStatus data = parseResponse(response, AnalyticsStatus.class);
		...
		if (showRetry)
		{
			eventBus.post(new AnalyticsRequestFailed(username, AnalyticsRequestType.COMPETITIONS_ONGOING));
		}
	}
	...
	private void playerUpcomingCompetitionsCallback(String username, Response response)
	{
		...
		if (response.isSuccessful())
		{
			...
			postEvent(new AnalyticsUpcomingPlayerCompetitionsFetched(username, comps));
			showRetry = false;
		}
		else if (response.code() == 429)
		{
			log.error("osrs-analytics: reached api limits while fetching upcoming competitions");
		}
		else
		{
			AnalyticsStatus data = parseResponse(response, AnalyticsStatus.class);
		...
		if (showRetry)
		{
			eventBus.post(new AnalyticsRequestFailed(username, AnalyticsRequestType.COMPETITIONS_UPCOMING));
		}
	}
	...
	public void commandLookup(String username, AnalyticsCommand command, ChatMessage chatMessage)
	{
		Request request = createRequest("players", username);
		sendRequest(request, r -> commandCallback(r, command, chatMessage));
	}

	private void commandCallback(Response response, AnalyticsCommand command, ChatMessage chatMessage)
	{
		if (!response.isSuccessful())
		{
			return;
	...
	public void updatePlayer(String username, long accountHash)
	{
		...
		Request request = createRequest(new AnalyticsPlayerUpdate(accountHash), "players", username);
		sendRequest(request);
	}
