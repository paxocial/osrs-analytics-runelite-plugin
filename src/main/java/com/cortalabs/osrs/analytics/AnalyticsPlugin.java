...
import com.cortalabs.osrs.analytics.beans.CanvasCompetition;
import com.cortalabs.osrs.analytics.beans.Competition;
import com.cortalabs.osrs.analytics.beans.NameChangeEntry;
import com.cortalabs.osrs.analytics.beans.ParticipantWithStanding;
import com.cortalabs.osrs.analytics.beans.GroupMembership;
import com.cortalabs.osrs.analytics.beans.ParticipantWithCompetition;
import com.cortalabs.osrs.analytics.events.AnalyticsGroupSynced;
import com.cortalabs.osrs.analytics.events.AnalyticsOngoingPlayerCompetitionsFetched;
import com.cortalabs.osrs.analytics.events.AnalyticsRequestFailed;
import com.cortalabs.osrs.analytics.events.AnalyticsUpcomingPlayerCompetitionsFetched;
import com.cortalabs.osrs.analytics.panel.CompetitionCardPanel;
import com.cortalabs.osrs.analytics.panel.NameAutocompleter;
import com.cortalabs.osrs.analytics.panel.AnalyticsPanel;
import com.cortalabs.osrs.analytics.ui.CodeWordOverlay;
import com.cortalabs.osrs.analytics.ui.CompetitionInfoBox;
import com.cortalabs.osrs.analytics.ui.SyncButton;
import com.cortalabs.osrs.analytics.util.DelayedAction;
import com.cortalabs.osrs.analytics.web.AnalyticsRequestType;
import com.cortalabs.osrs.analytics.web.AnalyticsClient;
import com.cortalabs.osrs.analytics.web.AnalyticsCommand;
...
@PluginDescriptor(
	name = "OSRS Analytics",
	tags = {"analytics", "group", "xp", "clan"},
	description = "Push OSRS account insights to cortalabs.com for opt-in tracking and competitions."
)
public class AnalyticsPlugin extends Plugin
{
	static final String CONFIG_GROUP = "osrsanalytics";
	...
	private static final String MENU_TARGET = "Analytics group";
	...
	private static String MESSAGE_PREFIX = "Analytics: ";
	...
	@Inject
	private AnalyticsClient womClient;
	...
	private AnalyticsPanel womPanel;
	...
	static
	{
		WORKING_DIR = new File(RuneLite.RUNELITE_DIR, "osrs-analytics");
		WORKING_DIR.mkdirs();

		try (InputStream inputStream = AnalyticsPlugin.class.getResourceAsStream("/version.ini"))
		{
	...
	{
		log.info("OSRS Analytics started! (v{})", pluginVersion);

		// This will work, idk why really, but ok
		womPanel = injector.getInstance(AnalyticsPanel.class);
		try
		{
			loadFile();
		}
		...
		for (AnalyticsCommand c : AnalyticsCommand.values())
		{
	...
	protected void shutDown() throws Exception
	{
		...
		log.info("OSRS Analytics stopped!");
	}
	...
	private void commandHandler(ChatMessage chatMessage, String s)
	{
		...
		AnalyticsCommand cmd = AnalyticsCommand.fromCommand(s);
	...
	private void commandLookup(AnalyticsCommand command, ChatMessage chatMessage)
	{
	...
	public void onMenuOpened(MenuOpened event)
	{
		...
		client.getMenu().createMenuEntry(-1)
			...
			.onClick(e -> {
				if (!rankIsIgnored)
				{
					chatboxPanelManager.openTextMenuInput("Are you sure you want to ignore " + rankTitle + " from analytics sync?")
	...
	private void compareClanMembersList(ClanSettings clanSettings)
	{
		...
		if (onlyInClan.isEmpty() && onlyInGroup.isEmpty())
		{
			...
			for (ClanMember cm : clanMembers)
			{
				...
				// clanTitle=null syncs to default role "member" on the analytics backend.
	...
	@Subscribe
	public void onAnalyticsGroupSynced(AnalyticsGroupSynced event)
	{
	...
	@Subscribe
	public void onAnalyticsOngoingPlayerCompetitionsFetched(AnalyticsOngoingPlayerCompetitionsFetched event)
	{
	...
	@Subscribe
	public void onAnalyticsUpcomingPlayerCompetitionsFetched(AnalyticsUpcomingPlayerCompetitionsFetched event)
	{
		// Filter out competitions with null metrics
	...
	@Subscribe
	public void onAnalyticsRequestFailed(AnalyticsRequestFailed event)
	{
		if (event.getType() == AnalyticsRequestType.COMPETITIONS_ONGOING || event.getType() == AnalyticsRequestType.COMPETITIONS_UPCOMING)
		{
	...
	public void configure(Binder binder)
	{
		...
		binder.bind(AnalyticsClient.class);
