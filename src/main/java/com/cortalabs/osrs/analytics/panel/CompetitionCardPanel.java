...
import com.cortalabs.osrs.analytics.beans.CompetitionProgress;
import com.cortalabs.osrs.analytics.beans.GroupInfo;
import com.cortalabs.osrs.analytics.beans.Metric;
...
import com.cortalabs.osrs.analytics.util.Format;
import com.cortalabs.osrs.analytics.util.Utils;
...
public class CompetitionCardPanel extends JPanel
{
	...
	CompetitionCardPanel(Client client, AnalyticsPlugin plugin, ParticipantWithStanding p)
	{
		this.client = client;
...
//		gainedLabel.setToolTipText(String.format("%,.1f", p.getProgress().getGained()));
	...
	CompetitionCardPanel(Client client, AnalyticsPlugin plugin, ParticipantWithCompetition p)
	{
		this.client = client;
		this.competition = p.getCompetition();
		this.plugin = plugin;
	...
	private JPanel createIconPanel(Metric metric)
	{
		...
		ImageIcon iconBg = new ImageIcon(ImageUtil.loadImageResource(AnalyticsPlugin.class, "icon_bg.png"));
