...
import com.cortalabs.osrs.analytics.beans.GroupInfo;
...
@Slf4j
public class AnalyticsPanel extends PluginPanel
{
	@Inject
	...
	private final AnalyticsClient womClient;
	...
	@Inject
	public AnalyticsPanel(Client client, AnalyticsPlugin plugin, NameAutocompleter nameAutocompleter, AnalyticsClient womClient, AnalyticsConfig config,
	...
	private void updateAfterSearch(String lookup, PlayerInfo result, Throwable ex)
	{
		SwingUtilities.invokeLater(() ->
		{
			...
			{
				log.warn("Player on analytics backend without snapshot {}.", lookup);
				searchBar.setIcon(IconTextField.Icon.ERROR);
				searchBar.setEditable(true);
	...
	public void addOngoingCompetitions(List<ParticipantWithStanding> competitions)
	{
		...
		for (ParticipantWithStanding c : competitions)
		{
			...
			if (plugin.competitionsOnCanvas.stream().anyMatch(cc -> cc.getId() == c.getCompetitionId()) ||
				config.addCompetitionsToCanvas().equals(AnalyticsConfig.CompetitionsToAddToCanvas.ONGOING) ||
				config.addCompetitionsToCanvas().equals(AnalyticsConfig.CompetitionsToAddToCanvas.BOTH))
			{
	...
	public void addUpcomingCompetitions(List<ParticipantWithCompetition> competitions)
	{
		...
		for (ParticipantWithCompetition c : competitions)
		{
			...
			if (plugin.competitionsOnCanvas.stream().anyMatch(cc -> cc.getId() == c.getCompetitionId()) ||
				config.addCompetitionsToCanvas().equals(AnalyticsConfig.CompetitionsToAddToCanvas.UPCOMING) ||
				config.addCompetitionsToCanvas().equals(AnalyticsConfig.CompetitionsToAddToCanvas.BOTH))
			{
				plugin.addInfoBox(competitionPanel);
			}
	...
	public void displayCompetitionFetchError(AnalyticsRequestType type, String username)
	{
		...
		competitionsErrorPanel.setVisible(false);
		if (type == AnalyticsRequestType.COMPETITIONS_UPCOMING)
		{
			upComingCompetitionsPanel.removeAll();
		...
		else if (type == AnalyticsRequestType.COMPETITIONS_ONGOING)
		{
			ongoingCompetitionsPanel.removeAll();
			retryButton.addActionListener(e ->
